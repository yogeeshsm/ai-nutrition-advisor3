...
class MalnutritionPredictor:
    ...
    def _generate_training_data(self, n_samples=1000):
        ...
        # Generate weights and heights with some correlation to age
        base_weight = 7 + (age_months / 12) * 1.5
        weight_kg = base_weight + np.random.normal(0, 2, n_samples)
        
        base_height = 65 + (age_months / 12) * 6
        height_cm = base_height + np.random.normal(0, 5, n_samples)
        
        # Calculate Z-scores
        wfa_zscore = np.random.normal(-0.5, 1.5, n_samples)
        hfa_zscore = np.random.normal(-0.5, 1.5, n_samples)
        wfh_zscore = np.random.normal(-0.5, 1.5, n_samples)
        
        # Growth velocities
        weight_velocity = np.random.normal(0.3, 0.15, n_samples)
        height_velocity = np.random.normal(0.7, 0.2, n_samples)
        
        # Diet features
        meals_per_day = np.random.randint(2, 6, n_samples)
        milk_intake = np.random.randint(200, 800, n_samples)
        protein_servings = np.random.randint(1, 4, n_samples)
        veg_servings = np.random.randint(1, 5, n_samples)
        
        # Health features
        illness_days = np.random.poisson(2, n_samples)
        diarrhea = np.random.choice([0, 1], n_samples, p=[0.85, 0.15])
...
def get_predictor():
    """Get the global malnutrition predictor instance"""
    return predictor
